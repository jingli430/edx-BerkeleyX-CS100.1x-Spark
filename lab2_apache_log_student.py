...
def parseApacheLogLine(logline):
    ...
    return (Row(                            # Row() is imported from pyspark.sql
        host          = match.group(1),
...
uniqueHosts = hosts.distinct() # better way to distinct???
...
# In Python you can get the unique items in a list by using a set
dayHostCount = dayGroupedHosts.map(lambda s: (s[0], len(set(s[1])))) #better solution?
# but set( ) function needs creating full iterable in memory, which is very expensive
...
#Solution2: avoiding using set()
#take tuple(day, host) as the new key --> reduceByKey will give you unique (day,host) --> unique hosts by key
#at the cost of losing info of host in dayGroupedHosts
# dayToHostPairTuple = access_logs.map(lambda log: ((log.date_time.day,log.host),1)) 
# dayGroupedHosts = dayToHostPairTuple.reduceByKey(lambda a,b:a+b).map(lambda ((k,v),cnt):(k,1)) 
# dailyHosts = dayGroupedHosts.reduceByKey(lambda a,b:a+b).sortByKey().cache()
# dailyHostsList = dailyHosts.take(30)
# print 'Unique hosts per day: %s' % dailyHostsList
...
plt.xlabel('Day')
plt.ylabel('Hosts')
plt.plot(daysWithHosts, hosts) #plot()
pass
...
sortedByDay = groupedByDay.map(lambda s: (s[0],len(s[1]))).sortByKey()
avgDailyReqPerHost = (sortedByDay
                      .zip(dailyHosts)
                      .map(lambda s: (s[0][0],s[0][1]/s[1][1]))  #solution2: len(s[1])/len(set(s[1]))
                      .cache()
                     )
...
# # Solution3: using the solution from 3c 
# dayAndHostTuple = access_logs.map(lambda log: ((log.date_time.day,log.host),1)) 
# # "v"-> total cnt, "1" -> unique cnt for each host within the same key
# # calc both "total cnt" and "unique cnt" at the same time for each key
# # format of tuple(v,1) should be retained after calc
# groupedByDay = (dayAndHostTuple.reduceByKey(lambda a,b:a+b)
#                                .map(lambda ((k,v),cnt):(k,(cnt,1)))
#                                .reduceByKey(lambda v1,v2: (v1[0]+v2[0], v1[1]+v2[1])))
# sortedByDay = groupedByDay.sortByKey()
# # v in tuple(k,v) actually is tuple(total cnt, unique cnt)
# avgDailyReqPerHost = sortedByDay.map(lambda (k,v):(k,v[0]/v[1])).cache()
# avgDailyReqPerHostList = avgDailyReqPerHost.take(30)
# print 'Average number of daily requests per Hosts is %s' % avgDailyReqPerHostList
...
#a&b are both values from different tuple for the same key, so they canNOT be parenthesised in lambda func
badEndpointsSum = badEndpointsCountPairTuple.reduceByKey(lambda a,b:a+b) 
...
errByDate = errDateSorted.collect() # as a list means using collect() to convert
...
topErrDate = errDateSorted.takeOrdered(5, key=lambda (k,v):-v)
